...
export function resolveFilename(template: string, values: ResolvedVariables): string {
	let rendered = renderTemplate(template, values, new Set(Object.keys(values)))
		.trim()
		.replace(/\s+/g, ' ');
	if (rendered.includes('/') || rendered.includes('\\')) throw new TemplateValidationError('Output filename cannot contain path separators.');
...
export function findAvailablePath(desiredPath: string, strategy: Exclude<FileConflictStrategy, 'prompt'>, exists: (path: string) => boolean): string {
