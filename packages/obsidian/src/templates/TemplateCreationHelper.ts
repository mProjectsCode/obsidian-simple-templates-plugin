import { isValidTemplateId, VaultPathHelper } from 'packages/core/src/index';
...
export class TemplateCreationHelper {
	...
	normalize(request: TemplateCreationRequest): TemplateCreationRequest {
		...
		if (!normalized.id) {
			throw new Error('Enter a template ID.');
		}
		if (!isValidTemplateId(normalized.id)) {
		...
		if (this.paths.hasPathSeparator(normalized.filename)) {
			throw new Error('File name cannot contain path separators.');
		}
		if (this.paths.hasUnsupportedFilenameCharacters(normalized.filename)) {
			throw new Error('File name contains unsupported characters.');
		}
		if (/[. ]$/.test(normalized.filename)) {
			throw new Error('File name cannot end with a dot or space.');
		}
		if (/^\.md$/i.test(normalized.filename)) {
			throw new Error('Enter a file name before the Markdown extension.');
		}
